...
const FONT_BPL: usize = 1;
const FONT_NUMGLYPHS: usize = 256;

const FONT_GLYPHS: [[u8; FONT_BPG]; FONT_NUMGLYPHS] = [
    ...
    unsafe {
      if buf[20] == 32 && buf[28] != 0 {
        FRAMEBUFFER = Framebuffer {
          fb: (buf[28] & 0x3FFFFFFF) as *mut u8,
          width: buf[10],
          height: buf[11],
          pitch: buf[33],
          isrgb: buf[24],
...
fn draw_pixel(x: u32, y: u32, attr: u8) {
  unsafe {
    assert!(!FRAMEBUFFER.fb.is_null());
    let offs = ((y * FRAMEBUFFER.pitch) + (x * 4)) as isize;
    *(FRAMEBUFFER.fb.offset(offs) as *mut u32) = VGA_PAL[(attr & 0x0f) as usize];
  }
}

fn draw_char(ch: u8, x: u32, y: u32, attr: u8) {
  for r in 0..FONT_HEIGHT {
    for c in 0..FONT_WIDTH {
      ...
      draw_pixel(x + (c as u32), y + (r as u32), px);
    }
  }
}
